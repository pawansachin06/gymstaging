...
class SlimHelper {
    ...
    private static function getPostData($inputName) {
        ...
        $values = array();
        if ($imageData = request($inputName)) {
            $values = $imageData;
        }
    ...
    public static function uploadImage($inputname, $filepath = '/')
    {
        $img = null;

        $images = self::getImages($inputname);

        if (isset($images) && is_array($images)) {
            $image = array_shift($images);
            if (isset($image['output']['data'])) {
                $filePath = $filepath. time() . "_" . $image['output']['name'];
                Storage::cloud()->put($filePath, $image['output']['data'], 'public');
                $img = Storage::cloud()->url($filePath);
            }
        }
