...
@section('content')
    ...
    <p>
        <ul class="list-inline">
            <li><a href="{{ route('admin.category.index') }}" style="{{ request('show_deleted') == 1 ? '' : 'font-weight: 700' }}">@lang('quickadmin.qa_all')</a></li> |
            <li><a href="{{ route('admin.category.index') }}?show_deleted=1" style="{{ request('show_deleted') == 1 ? 'font-weight: 700' : '' }}">@lang('quickadmin.qa_trash')</a></li>
        </ul>
    </p>
    @endcan
                        ...
                        @foreach ($categories as $category)
                            <tr data-entry-id="{{ $category->id }}">
                                ...
                                <td field-key='businesses'>
                                    <span class="label label-info label-many">{{ $category->business->label }}</span>
                                </td>
...
@section('javascript') 
    <script>
        @can('category_delete')
            @if ( request('show_deleted') != 1 ) window.route_mass_crud_entries_destroy = '{{ route('admin.category.mass_destroy') }}'; @endif
        @endcan
