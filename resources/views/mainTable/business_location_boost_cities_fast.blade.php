            ...
            <div class="row justify-content-md-center">
                <div class="col-12 col-md-12 col-lg-12 col-xl-8 mb-1 mt-4">
                    ...
                    <div class="glass-borders br-20" style="padding: 20px 60px 20px 60px;">
                            ...
                            <div class="col-12">
                                ...
                                <div>
                                    <div class="input-group">
                                        <input type="text" id="address-autocomplete-input" placeholder="Type address here..." spellcheck="false" autocomplete="off" class="form-control shadow-sm" />
                                        <div class="input-group-append">
                                            <button id="address-autocomplete-clear-btn" type="button" class="btn btn-outline-secondary">
                        ...
                        <div id="boostedCitiesRow" class="row mb-3"></div>

                        @if(false)
                            <form id="location-boost-cities-form" action="{{ route('business.dolocationboost') }}">
                                <div class="row justify-content-center">
                                    <div class="col-12 col-md-12 col-lg-12 col-xl-12 payment-right ml-4">
                                            ...
                                            <div class="col-sm-6">
                                                ...
                                                <select class="form-control" id="country">
                                                    ...
                                                    @foreach($countries as $key => $name)
                                                        <option data-key="{{ $key }}" value="{{$name}}">{{$name}}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                            <div class="col-sm-6">
                                                ...
                                                <select class="form-control" id='city'>
                                                    ...
                                                    @if(!empty($simpleCities))
                                                        @foreach($simpleCities as $key => $city)
                                                            @if(!empty($city))
...
<script type="text/javascript">
    ...
    var cityLocations = JSON.parse('<?php echo json_encode($cities); ?>');
    ...
    (function($) {
        ...
        $('#country').on('change', function () {
            $('#city').html('');
            $.ajax({
                url: "{{ route('business.getcities') }}?country="+this.value+'&key=GB',
                type: 'get',
                success: function (res) {
    ...
    function setupAutocompleteAddressInput() {
        ...
        if(autocompleteAddressInput && autocompleteAddressPredictions) {
            handleAddressSelection(autocompleteAddressInput, autocompleteAddressPredictions);
    ...
    function addressAutocompleteCallback(predictions, status, predictionsEl){
        ...
        if(predictions.length){
            ...
            for (var i = 0, prediction; prediction = predictions[i]; i++) {
                ...
                div.classList.add('address-autocomplete-prediction');
            ...
            geocoder.geocode({'address': address}, function (results, status) {
                if (status == google.maps.GeocoderStatus.OK) {
                    ...
                    if(!addressParts.city.length) {
                        ...
                        handleAddCity(addressParts.country, addressParts.city, lat, lang);
                    }
                    // console.log(addressParts, lat, lang);
                }
            });
        } catch (e) {
            console.log(e);
    ...
    function handleAddCity(country, city, lat, lang){
        const found = boostCities.some(function(el) {
            return (el.city === city && el.country == country );
        });
        if(found) {
            toast.error('This city already exist. Please choose other city!')
            return;
        }
        var addedCity = false;
        for (var i = 0; i < boostCities.length; i++) {
            if(boostCities[i].latitude == null) {
                boostCities[i].city = city;
                boostCities[i].country = country;
                boostCities[i].latitude = lat;
                boostCities[i].longitude = lang;
                addedCity = true;
                break;
            }
        }
        if(addedCity) {
            ...
            toast.error('Can’t add city, upgrade plan or delete a existing location');
        }
    }

    boostedCitiesRow.addEventListener('click', function(e){
        if(e?.target?.classList.contains('btn')) {
            let id = e.target.getAttribute('data-id');
            if(id.length) {
                for (var i = 0; i < boostCities.length; i++) {
                    if(id == boostCities[i].id) {
                        boostCities[i].city = null;
                        boostCities[i].country = null;
                        boostCities[i].latitude = null;
                        boostCities[i].longitude = null;
                        break;
                    }
                }
                updateBoostedCitiesRow();
    ...
    function updateBoostedCitiesRow(){
        var boostedCitiesRow = document.getElementById('boostedCitiesRow');
        while (boostedCitiesRow.firstChild) {
            boostedCitiesRow.removeChild(boostedCitiesRow.lastChild);
        }
        for (var i = 0; i < boostCities.length; i++) {
            let inputEl = document.createElement('input');
            inputEl.classList.add('form-control', 'shadow-sm');
            inputEl.type = 'text';
            let txt = (boostCities[i].city ? boostCities[i].city + ', ' : '') + (boostCities[i].country ? boostCities[i].country : '');
            inputEl.value = txt.length ? txt : 'City #' + (i + 1);
            inputEl.readOnly = true;

            let inputButtonIcon = document.createElement('i');
            inputButtonIcon.classList.add('fa','fa-times');
            inputButtonIcon.style.pointerEvents = 'none';

            let inputButton = document.createElement('button');
            inputButton.classList.add('btn', 'btn-outline-danger', 'border-danger');
            inputButton.appendChild(inputButtonIcon);
            inputButton.setAttribute('data-id', boostCities[i].id ? boostCities[i].id : '');

            let divGroupAppend = document.createElement('div');
            divGroupAppend.classList.add('input-group-append');
            divGroupAppend.appendChild(inputButton);

            let divInputGroup = document.createElement('div');
            divInputGroup.classList.add('input-group');
            divInputGroup.appendChild(inputEl);
            divInputGroup.appendChild(divGroupAppend);

            let divCol = document.createElement('div');
            divCol.classList.add('col-12', 'col-sm-6', 'mb-3');
            divCol.appendChild(divInputGroup);
            boostedCitiesRow.appendChild(divCol);
        }
        window.initMap();
    }
    ...
    function initMap() {
        if(service === null) initMapService();
                ...
                if(boostCities[i].latitude !=null && boostCities[i].longitude !=null){
                    marker = new google.maps.Marker({
                        ...
                        icon: {
                            ...
                            // url: "{{ url('gymselect/images/bubble.png') }}"
                        },
                    });
                    google.maps.event.addListener(marker, 'click', (function(marker, i) {
                        return function() {
                            infowindow.setContent('<div class="px-2 py-2 font-weight-bold">' + boostCities[i].city + ', <br>' + boostCities[i].country + '</div>');
...
<script src="https://maps.google.com/maps/api/js?key={{ env('GOOGLE_GEOLOCATION_API') }}&callback=initMap&loading=async&libraries=places" ></script>
